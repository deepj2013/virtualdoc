...
import { UserGroupIcon, PlusIcon, MagnifyingGlassIcon, PencilIcon } from '@heroicons/react/24/outline';
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1">Manage admin users and their access levels</p>
        </div>
          ...
          <>
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {admins.map((admin) => (
                    <tr key={admin.id} className="border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                      <td className="py-3 px-4">
                        <div className="font-medium text-gray-900 dark:text-white">
                          {admin.firstName} {admin.lastName}
                        </div>
                      </td>
                      <td className="py-3 px-4 text-gray-600 dark:text-gray-400">{admin.email}</td>
                      <td className="py-3 px-4">
                        <span className="px-2 py-1 text-xs font-medium rounded-full bg-indigo-100 dark:bg-indigo-900/30 text-indigo-800 dark:text-indigo-300">
                          {admin.roleName || admin.adminRoleCode}
                        </span>
                      </td>
                      <td className="py-3 px-4">
                        {admin.isSuspended ? (
